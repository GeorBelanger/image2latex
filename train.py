...
def train(images, targets, targets_eval, cnn, encoder, decoder, cnn_optimizer, encoder_optimizer, decoder_optimizer, criterion, max_length):
	...
	m = nn.Tanh()
	decoder_hidden = m(torch.mm(img_mean.squeeze(), W1)+b1.transpose(0,1).expand_as(torch.mm(img_mean.squeeze(), W1)))

	#Calculate the first cell state of the decoder LSTM
	W2 = Variable(torch.zeros(512, 512))
	b2 = Variable(torch.zeros(512, 1))  
	decoder_cell_state = m(torch.mm(img_mean.squeeze(), W2)+b2.transpose(0,1).expand_as(torch.mm(img_mean.squeeze(), W2)))
